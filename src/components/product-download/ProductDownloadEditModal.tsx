...
import { useUpdateProductDownload } from "@/hooks/useProductDownload";
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
import { Loader2, X, FileText, Image as ImageIcon, File, Download } from "lucide-react";
import type { ProductDownload } from "@/types/IDownload";
import { useRecaptcha } from "@/hooks/useRecaptcha";
import { Icon } from "@iconify/react/dist/iconify.js";

interface ProductDownloadEditModalProps {
  open: boolean;
...
export function ProductDownloadEditModal({
  open,
  onOpenChange,
  download,
  productId,
  categoryId
}: ProductDownloadEditModalProps) {
  const updateMutation = useUpdateProductDownload();
  ...
  useEffect(() => {
    if (download) {
      ...
      // Set platform input as comma-separated string
      setPlatformInput(download.platforms.join(', '));
    }
  }, [download]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Use the same recaptcha token logic as in ProductDownloadCreateModal
    ...
    try {
      await updateMutation.mutateAsync({
        ...
        data: {
          ...
          sizeBytes: formData.downloadUrl ? formData.sizeBytes : undefined,
      ...
      toast.error(error?.response?.data?.message || "Failed to update download");
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Edit Product Download</DialogTitle>
          <DialogDescription>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              className={errors.downloadUrl ? 'border-red-500' : '' + " mt-2"}
            />
            {errors.downloadUrl && (
              ...
              <div className="mt-4 p-4 rounded-lg bg-muted">
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="mt-2 space-y-1 text-xs text-gray-600">
                      <p>Size: {(download.sizeBytes / 1024 / 1024).toFixed(2)} MB</p>
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={updateMutation.isPending || Object.keys(errors).length > 0}>

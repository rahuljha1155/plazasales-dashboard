...
export default function BrandSeoList() {
    ...
    const tableData = brandId
        ? (data?.data?.records || []).filter((seo) => seo.entityId === brandId)
        : data?.data?.records || [];
                    ...
                    elements={
                        <div className="flex gap-2">
                            ...
                            {brandId && (
                                <Button
                                    className="rounded-sm hover:shadow-md transition-shadow"
                                    onClick={() => navigate(`/dashboard/brands/${brandId}/seo/create?entityType=BRAND&entityId=${brandId}`)}
                                >

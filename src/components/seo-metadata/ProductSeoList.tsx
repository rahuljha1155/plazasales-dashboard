...
export default function ProductSeoList() {
    ...
    // Filter SEO records for this specific product if productId is provided
    const tableData = productId
        ? (data?.data?.records || []).filter((seo) => seo.entityId === productId)
        : data?.data?.records || [];
                    ...
                    elements={
                        <div className="flex gap-2">
                            ...
                            {productId && (
                                <Button
                                    className="rounded-sm hover:shadow-md transition-shadow"
                                    onClick={() => navigate(`/dashboard/products/${productId}/seo/create?entityType=PRODUCT&entityId=${productId}`)}
                                >

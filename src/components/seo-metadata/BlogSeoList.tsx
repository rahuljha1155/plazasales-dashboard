...
export default function BlogSeoList() {
    ...
    const tableData = blogId
        ? (data?.data?.records || []).filter((seo) => seo.entityId === blogId)
        : data?.data?.records || [];
                    ...
                    elements={
                        <div className="flex gap-2">
                            ...
                            {blogId && (
                                <Button
                                    className="rounded-sm hover:shadow-md transition-shadow"
                                    onClick={() => navigate(`/dashboard/blogs/${blogId}/seo/create?entityType=BLOG&entityId=${blogId}`)}
                                >

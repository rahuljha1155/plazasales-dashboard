...
export default function SubcategorySeoList() {
    ...
    const tableData = subcategoryId
        ? (data?.data?.records || []).filter((seo) => seo.entityId === subcategoryId)
        : data?.data?.records || [];
    ...
    return (
        <div className="w-full space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-6">
                <Breadcrumb
                    links={[
                        { label: "Subcategories", isActive: false },
                    ...
                    elements={
                        <div className="flex gap-2">
                            ...
                            {subcategoryId && (
                                <Button
                                    className="rounded-sm hover:shadow-md transition-shadow"
                                    onClick={() => navigate(`/dashboard/subcategories/${subcategoryId}/seo/create?entityType=SUBCATEGORY&entityId=${subcategoryId}`)}
                                >
